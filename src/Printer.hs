...
prettyStructInt intGetter opt content =
  let intVal = (fromIntegral $ runGet intGetter (BSL.fromStrict content)) :: Integer 
      intStr = 
...
prettyStructValue :: Options -> ValueType -> BS.ByteString -> BSB.Builder
prettyStructValue opt vt content = 
...
prettyIndex :: Context -> BSB.Builder
prettyIndex context =
  case contextPath context of
...
prettyStructLine opt context st content =
  let size = structSize st
      helper next c s d =
        case s of
          Mono vt   -> prettyIndex c <> prettyStructValue opt vt d             <> BSB.string7 ", "
          Array _ _ -> prettyIndex c <> BSB.char8 '[' <> next <> BSB.char8 ']' <> BSB.string7 ", "
          Struct _  -> prettyIndex c <> BSB.char8 '{' <> next <> BSB.char8 '}' <> BSB.string7 ", "
          Inline _  -> next
